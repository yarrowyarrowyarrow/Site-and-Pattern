...
"the contract is a test" discipline as test_architecture_guard.py:
  ...
  1. The philosophy doc exists and documents all thirteen core themes.
  2. Every ``Design principle P#`` anchor in src/ names a real principle (1–13),
...
class TestPhilosophyDoc(unittest.TestCase):
    ...
    def test_doc_documents_all_thirteen_themes(self):
...
class TestCodeAnchors(unittest.TestCase):
    def test_anchors_name_real_principles(self):
        ...
        for path in _SRC.rglob("*.py"):
            ...
            for num in found:
                if not (1 <= int(num) <= 13):
                    offenders.append(f"{path}: P{num} is not a real principle (1–13)")
