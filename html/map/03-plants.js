...
// globals is pinned by tests/test_map_js.py + tests/test_bridge_contract.py.
    ...
    // Recolour placed-plant markers as a chosen bee's resource map: its hosts
    // glow like nectar (graded by tongue-fit for bumble bees), others grey out.
    // Non-destructive — clearBeeForageView recomputes each marker's normal style.
    // Driven from the Bees tab via map_js.set_bee_forage_view / clear_bee_forage_view.
    ...
    function _beeForageLegend(beeLabel) {
      var el = document.getElementById('bee-forage-legend');
      if (beeLabel === null) { if (el) el.style.display = 'none'; return; }
      if (!el) {
    ...
    function setBeeForageView(beeLabel, styles) {
      ...
      Object.keys(plantMarkers).forEach(function(mid) {
        ...
        c.setStyle(key ? (_BEE_FORAGE_STYLE[key] || _BEE_FORAGE_STYLE.host) : _BEE_FORAGE_DIM);
      });
      _beeForageLegend(beeLabel);
    }
