...
// globals is pinned by tests/test_map_js.py + tests/test_bridge_contract.py.
    ...
    // ── "What the bee sees" — floral-resource map (F37 increment 3) ────────
    // Recolour placed-plant markers as a chosen bee's resource map: its host
    // plants glow like nectar (graded by tongue-fit for bumble bees), every
    // other plant recedes to grey. Non-destructive — clearBeeForageView
    // recomputes each marker's normal plant-type style. Driven from the Bees
    // tab via map_js.set_bee_forage_view / clear_bee_forage_view.
    var _BEE_FORAGE_STYLE = {
      good:      { color: '#ffb300', fillColor: '#ffd54f', fillOpacity: 0.82, weight: 3 },
      plausible: { color: '#ffca28', fillColor: '#ffe082', fillOpacity: 0.62, weight: 2.5 },
      host:      { color: '#ffd740', fillColor: '#fff176', fillOpacity: 0.66, weight: 2.5 }
    };
    var _BEE_FORAGE_DIM = { color: '#5b6b7a', fillColor: '#5b6b7a', fillOpacity: 0.10, weight: 1 };

    function _normalPlantStyle(pd) {
      var color = (pd && pd.customColor) ||
                  (pd && TYPE_COLORS[pd.plantType]) || '#66bb6a';
      return { color: color, fillColor: color, fillOpacity: 0.35, weight: 1.5 };
    }

    function setBeeForageView(beeLabel, styles) {
      styles = styles || {};
      Object.keys(plantMarkers).forEach(function(mid) {
        var c = plantMarkers[mid];
        if (!c || !c._pd || !c.setStyle) return;
        var key = styles[String(c._pd.plantId)];
        c.setStyle(key ? (_BEE_FORAGE_STYLE[key] || _BEE_FORAGE_STYLE.host)
                       : _BEE_FORAGE_DIM);
      });
      _showBeeForageLegend(beeLabel);
    }

    function clearBeeForageView() {
      Object.keys(plantMarkers).forEach(function(mid) {
        var c = plantMarkers[mid];
        if (c && c._pd && c.setStyle) c.setStyle(_normalPlantStyle(c._pd));
      });
      _hideBeeForageLegend();
    }

    function _showBeeForageLegend(beeLabel) {
      var el = document.getElementById('bee-forage-legend');
      if (!el) {
        el = document.createElement('div');
        el.id = 'bee-forage-legend';
        el.style.cssText = 'position:absolute;left:10px;bottom:12px;z-index:1000;' +
          'font-family:system-ui,sans-serif;font-size:12px;color:#3a2a00;' +
          'background:rgba(255,248,225,.94);border:1px solid #ffd54f;border-radius:8px;' +
          'padding:6px 10px;line-height:1.5;pointer-events:none;' +
