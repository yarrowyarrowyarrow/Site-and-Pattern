...
| **Per-species ecoregion range** (schema v59) | [GBIF](https://www.gbif.org/) occurrence search — georeferenced records intersected with the polygons above by `scripts/seed_ecoregion_ranges.py`. **Only the derived counts are published**, never GBIF's records themselves: each row is "N georeferenced records fall inside this region", which is a number we computed, not a dataset we redistribute. Every row ships its count, a confidence band and the retrieval date. **V2.75: the raw points are now cached locally** in `data/fetched/plant_occurrences.json` so a re-derivation costs no network — a dev artefact, not shipped to users and not published to the website. Drawing those points on a public page would change this row and has not been done: see the "Publish the occurrence points" backlog entry for the licence and the rare-species-location question | GBIF search API is open; the derived counts are ours. The cached points are GBIF's records and are not redistributed |
