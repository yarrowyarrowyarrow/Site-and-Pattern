...
| V2.42 | [biological-data-review](V2.42-biological-data-review.md) | Audit the biological data: where the gaps are, and where the citations and sources are — flora↔fauna relationships especially | Review shipped ([DATA_AUDIT.md](../DATA_AUDIT.md)); the four app-changing parts scoped and held for the author. The suspicion was wrong in an interesting way: all 361 edges are properly cited, and not one citation is visible anywhere in the UI. Two existing docs were found making claims the code does not support. |
