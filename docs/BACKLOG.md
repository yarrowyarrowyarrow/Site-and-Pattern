...
**Totals: 32 code features · 13 data jobs · 4 legacy-ledger items.**
...
| **Re-derive the ranges without the buffer** | **The shipped `plant_ecoregions.json` counts are wrong and cannot be fixed here.** V2.75 found that `ranges_for_species` inherited `ecoregion._NEAR_BOUNDARY_M`, a 5 km proximity buffer meant for site detection, and credited every record to every region within five kilometres — 16.4% of points inside the layer land in two or more. The derivation is fixed; the *data* needs one re-fetch with egress. `python scripts/seed_ecoregion_ranges.py` (a few hours, politely throttled) also writes the point cache, after which the buffer, the floor and the uncertainty filter all become offline knobs and this class of bug costs nothing to re-check. **Then run `python scripts/plot_occurrences.py --buffer-artefacts`** and report the number — it is the honest measure of what the bug cost, and `/method/` currently discloses the buffer as a known limit until it does |
| **Ask VASCAN about nativity and names** | **The machinery shipped in V2.75 (F137); the fetch needs a machine with egress.** `python3 scripts/fetch_flora_nativity.py --probe` first — three requests, and it prints what percentage of records populate each field, because the response shape is a documented guess and V2.59 lost an afternoon to a fetch whose test fixture encoded the same wrong guess as the code. Then the real run (~430 requests) and `scripts/ingest_flora_nativity.py`, which **reports and applies nothing**. Expect the largest bucket to be `narrow`: 355 of 430 rows say exactly "AB,SK" and that string was generated from ecoregion tags, not read from a flora |
| **The Old World binomials** | Confirmed cases, from an outside botanical review: `Achillea millefolium` ships **beside its own segregate** `Achillea borealis`, both claiming AB,SK; `Fragaria vesca`, `Prunella vulgaris` and `Juniperus communis` ship as bare Eurasian binomials with no infraspecific rank; `Aster alpinus`; `Deschampsia caespitosa` where FNA and POWO use `cespitosa`, so a lookup on the accepted spelling misses. The catalogue has **no authority field, no synonym list and no taxon key** — a scientific name is a free string checked by one regex. VASCAN's accepted-name output (above) is the input to this. Renaming is not a data job: it moves plant ids, the `plant_fauna_master.json` keys that name a plant by common name, and public URLs. Its own increment |
| **Merge the synonym duplicates** | *Solidago rigida* and *Oligoneuron rigidum* are one taxon shipping as two published species pages that **disagree about nativity** — AB,SK native versus `native_to_alberta = 0`, SK,MB. Known since V2.69, deferred as "a data decision", and since V2.75 a *recorded* exception in `data_quality.KNOWN_NATIVITY_CONFLICTS` rather than an invisible one: any new contradiction is an error, this pair is a warning carrying its reason. Also `Solidago glutinosa` / `Solidago simplex`, both named "Sticky Goldenrod". Blocked on choosing an accepted name, which is what VASCAN answers |
...
| **Publish the occurrence points** | `scripts/plot_occurrences.py` draws them; nothing publishes them. Blocked on two questions, neither of them effort. **Licence**: `DATA_SOURCES.md` rests the GBIF position on storing "only the derived counts, never GBIF's records themselves", and drawing coordinates changes that sentence; records carry per-record licences. **Harm**: iNaturalist obscures coordinates for rare, threatened and collectible taxa precisely so publishing them does not lead a collector to the plant, and this catalogue has orchids and cacti in it. A published map must never plot a point finer than its source published it. Note the irony — the rare species the three-record floor under-serves are the same ones whose points must stay coarse. Until then, every species page links out to the live GBIF and iNaturalist maps, which is the free half |
