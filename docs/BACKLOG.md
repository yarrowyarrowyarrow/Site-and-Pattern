...
| **Publish the occurrence points** | `scripts/plot_occurrences.py` draws them and **V2.77 answered the licence half**: `fetch_dataset_licences.py` asked GBIF per dataset (224 of them) and got 61 CC0, 82 CC-BY, 81 CC-BY-NC. `--specimens --publishable` renders the layer the printed regional floras plot — **52,924 of 555,477 records drawable, 300–700 dots per species** — and it clears both original blockers rather than arguing with them: for a herbarium the dataset licence IS the record's licence, and a pressed sheet with a locality on its label carries none of the rare-taxa obscuring that iNaturalist applies for good reason. **V2.78 also stopped the maps drawing British Columbia** — the GBIF query is bounded by the polygon bounding box plus half a degree, and 175,876 records (31.7% of the harvest) were being drawn over ground the layer has no authority over; the drawable layer is 29,993 records once that and the specimen and licence filters all apply, and 20 of 434 species have nothing drawable. **What is left is a decision, not effort.** (a) Whether the dots reach the website at all, which needs a shipped point file (the cache is a dev artefact), a `/method/` disclosure and a `DATA_SOURCES.md` row. (b) Whether the **CC-BY-NC observations** join them — one line in `PUBLISHABLE` plus a row explaining why the occurrence rule differs from the photograph rule, where NC is refused because a photo is redistributed as a work and a coordinate is a fact about a place. Held so it is decided beside the specimen map rather than instead of it. Note the irony the review's other point turns on: the rare species the three-record floor under-serves are the same ones whose *observation* coordinates must stay coarse |
