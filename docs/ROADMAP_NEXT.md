...
working across both documents. **Next free ID: F152.**
...
| **F151** | A border read closer than it is drawn | ✅ **V2.81** — plan: [`V2.81-a-border-read-closer-than-it-is-drawn`](plans/V2.81-a-border-read-closer-than-it-is-drawn.md). Two species pages with every occurrence dot in the Rockies and Aspen Parkland on the region list: *“Aspen parkland cannot be absorbing these mountain native species… I’d prefer no overlap”*. **The overlap was not the cause.** *Penstemon albertinus* published 284 montane records and 17 parkland ones; of its 547 cached points exactly **one** is in two regions, so 6% could not be the 0.81% sliver V2.78 measured. Three hypotheses died first — no leftover buffer, no mislabelled sliver (a 6.5 km² five-vertex fragment does sit out there and looked like the answer until a transect showed it is a rounding artefact on the edge of a real lobe), and the boundary is not misplaced (the ELC parkland genuinely reaches the foothills). What it was: **the seventeen records are one population**, 663 m by 291 m, thirteen spots on one hillside, **25–202 m** inside a line the site’s own caveat calls accurate to a kilometre. The pipeline printed that caveat under every map and then asked the outline which side of itself a point 100 m away fell on — P9 failing in the same file that had already caught the 5 km version of it in V2.75. **The rule**: a record counts only where it lies more than `SIMPLIFICATION_M` (900 m, the number `CAVEAT` already prints in words) inside a region, measured to the nearest **different** region and never to a ring, because the layer splits each ecoregion by Alberta subregion and Aspen Parkland alone is 9 features and 65 parts with seams through the middle. **The overlap fix falls out for free**: polygons overlap *because* a shared border was simplified twice, so a doubly contained point is by construction inside the margin, and the double count is structurally impossible rather than disclosed. Cost: 5.7% fewer records credited, 3.6% set aside as too near a line, **108 region rows removed across 94 species and zero added** — a strict subset, which is what says it is conservative. Shipped envelope to version 2 carrying `boundary_margin_m` and a tally, so the Method page and the species caption publish the discard rather than restating a measured number. Left alone: `lookup_ecoregions` keeps its 5 km buffer, `subject_area` keeps plain containment, and **17 collections of one plant are still 17 records** — counting places rather than records is its own increment |
| **F150** | A link to the published site unfolds into a grey box with a URL under it, on a catalogue whose whole argument is that the plants are worth looking at | ✅ **V2.80** — plan: [`the-copy-in-the-authors-voice`](plans/V2.80-the-copy-in-the-authors-voice.md). Every page carried a `<title>` and a `<meta name="description"` and nothing else, so Facebook, Reddit, Slack, Discord and iMessage had nothing to show. **The trap is that this is invisible from the page**: the HTML validates, the site renders, nothing errors, and the only way to find out is somebody else pasting the link. Three consequences shaped `src/site_share.py`. **`og:image` must be absolute** — a scraper fetches it from its own servers with no page to resolve a relative path against, so `assets/photos/x.jpg` silently produces no card at all; that makes `--base-url` load-bearing for a third distinct job after the sitemap and the CNAME, and a build without one emits **no image tag rather than a broken one**, and says so in the build output. **Two vocabularies**, `og:*` and `twitter:*`, one set of values. **The credit rides in `og:image:alt`**, because these are open-licensed photographs, attribution is the licence condition, and inside somebody else's app the page carrying the credit is reduced to a link. Per page where a page has its own photograph (sharing a species page shows *that* species), Prairie Crocus otherwise — picked from the **built model**, not pinned to a file, so the card can never show a photograph the catalogue has stopped publishing. Its own module on the V2.73 precedent: the renderer was at 744 of 800 and this is not rendering, it is what the build tells other people's machines. **No image is generated** (a composed 1200×630 card would put the site build behind Pillow, which this project keeps optional on purpose) and **no `og:url`** (it would need every page's path threaded through `_page`'s call sites in six modules, and every scraper falls back to the URL it fetched) |
| **F145–F149** | A range is not made of ecoregions · shared geometry · publish the points · a missing VASCAN field · read the checklist instead of asking the API | 🚧 **V2.79–V2.80, in flight** — plans: [`V2.79-the-range-stops-being-ecoregions`](plans/V2.79-the-range-stops-being-ecoregions.md), [`V2.80-the-range-was-never-on-the-page`](plans/V2.80-the-range-was-never-on-the-page.md). **F145** ✅: V2.75–V2.78 each corrected *how* the ecoregion shading was derived and left the assumption underneath alone, until the author looked at the corrected maps — *“this range does not neatly conform to ecoregions”*. It does not, because it is not made of them; shading 100,000 km² because three records fall inside it is the same overstatement one level up. `src/species_range.py` bins the records into a **0.25° occupancy grid**: 426 species, **56,513 cells, 0.86 MB**, the whole catalogue's range data smaller than one of today's species pages. A grid rather than an alpha hull because a hull draws boundary through unrecorded ground *and* would put the site build behind `shapely`. `src/range_map.py` renders it in **91 KB against the ecoregion map's 846 KB**, specimens filled and observations hollow so the split survives greyscale and print. **V2.80 found why the three palettes read identical, and it was not the marks**: `range_svg` redraws the subject provinces over the range so a border stays legible against it, and that pass reused `.subj`, whose fill is opaque white — the wash had **never been on the page**, so palettes differing only in the wash rendered byte-identically apart from their stylesheet, and a fourth would have too. Nothing raises and the SVG is well-formed; the failure is entirely paint order, in a module that shipped with **no test file and no caller** because it existed to make PNGs for the author. Rendering it standalone found a second unstyled layer: `water_svg` and `cities_svg` emit `ecomap-*` classes that live in `site.css`, so every lake drew **black** and every river not at all, and the `water`/`river`/`city` entries in all three palettes **had never been applied to anything**. The recommended change then landed on a visible wash: `cell_counts` keeps the count `occupied_cells` discarded, banded 1 / 2-4 / 5-19 / 20-99 / 100+ — log-ish because **51% of the catalogue's 56,513 cells hold exactly one record** — with marks **off by default** and `marks="all"` left as the switch F147's toggle needs. The ramp is recording effort as much as plant, so the disclosure sits in three places and a test forbids *abundance*, *abundant*, *common* and *density* anywhere in a rendered SVG. Shipped file to version 2, `[lat, lng, records]`, 0.86 -> **0.98 MB**; a version 1 row reads as one record, not zero. **F146 open**: a species page is 888 KB of which **846 KB is the ecoregion map**, 2,291 polygons repeated on all 430 pages, built site **421 MB** — one cached geometry file takes pages to ~40 KB and makes the toggle natural rather than bolted on. **F147 open**: publish both record layers with a specimens/observations toggle, 539,129 publishable points at ~14 KB per species; the author has settled that `CC_BY_NC` is publishable **for coordinates only** (the photograph rule is untouched — a photograph is redistributed as a work, a coordinate is a fact about a place) and that the five collectible taxa are published as-is. **F148** ✅: the nativity fetch ran clean and reported **182 species not recorded from Alberta or Saskatchewan** — 42% of a native plant catalogue, *Amelanchier alnifolia* among them. VASCAN attaches distribution to the **lowest accepted taxon**, so a species with accepted varieties carries none, and `assess()` let the missing block fall through to *absent*. `not_here` **182 → 9**, fixed by `--reassess` over the fetched file with no network. Third instance here of *a failure is not an absence*. **F149** ✅: `search.json` is exact-name only (`Amelanchier alnifolia var` → `numMatches: 0`) and `parentNameUsageID` points the wrong way, so children cannot be enumerated — and the autonym shortcut is a **trap**, resolving *Amelanchier* and missing *Alnus incana* entirely, whose Alberta taxon is `subsp. tenuifolia`. `scripts/vascan_archive.py` reads the Darwin Core Archive and rolls accepted descendants up; **no download URL is in the code**, because a URL asserted from memory that fetches the wrong dataset *and parses* is the failure hardest to notice. **V2.80: the author ran it, and the reader refused the real archive** — `distribution.txt` is a Darwin Core extension and joins on `id`, not `taxonID`, which V2.79's synthetic archive had written in both files and so could never catch. Opened, `review` went **182 → 9** and one bug remained: VASCAN files a hybrid formula at rank species, so *Chamaenerion angustifolium* tied with *C. angustifolium* subsp. *angustifolium* × *C. latifolium* and **dict iteration order** picked the hybrid, publishing fireweed as unrecorded in Alberta. Fixed, plus `--explain` to see inside an archive no session here can open. Final: **380 confirm, 34 narrow, 11 not_here, 6 undetermined, 3 name**. **F146 and F147 then closed together, and F146 was backwards**: the handover listed shared geometry as *blocking* the range map, when swapping the range map in for the ecoregion map IS the fix -- the built site went **421 MB -> 118 MB** and a species page **888 KB -> 111 KB**, because 846 KB of that was 2,291 ecoregion polygons repeated on all 430 pages. The ecoregion counts stay (a count is a fact; only the picture drawn from it overclaimed). **F147** ships 171,896 marks with a specimen/observation toggle -- three radios and no JavaScript -- and implements the author's V2.79 licence decision: of 365,092 in-province, sufficiently precise records **329,267 are CC_BY_NC observations**, so the photograph bar would have published a map that is 94% herbarium specimens and called it the observation record. And the nativity inference is now **withheld rather than annotated** on the author's instruction -- *"only facts backed by data"* -- so 16 species read *Not established* and print no province list |
| **F142–F144** | The maps draw British Columbia; there is no bloom bar; the field the site is named for has no provenance mark | ✅ **V2.78** — **F142**: the GBIF query is bounded by the polygon *bounding box* plus half a degree and `map_svg` emits `overlay` outside its subject clip, so **175,876 records (31.7%)** were drawn over BC, Montana, Manitoba and the NWT. Kept honest at the edge by testing the coarse province outline **or** any surveyed ecoregion, because 193 vertices cannot adjudicate the continental divide. Also: the radius clamps were absolute units with 360 of 422 dots at the floor, and two versions shipped with no legend over three simultaneous encodings. **F140's pass then ran for real**: purely additive, 15 new region rows and none dropped, schema 77 -> **78**, and every new region northern because specimen share is 92.4% in Athabasca Plain against 2.7% in Fescue Grassland — the cap was dropping the north, not just the old. **F143**: the phenology bar on 424 of 430 species pages, fruit on the same axis, nothing recorded drawing nothing. **F144**: VASCAN still 403s at the proxy, so instead the nativity claim finally carries the mark every other field has — 354 of 430 species publish "AB, SK" from an ecoregion inference and the retired generator's own docstring says the review was reading its output. Derived rather than stored, with a test that fails the day the sourced column lands . **F144's seam filled in V2.80**: the author's VASCAN run put `native_provinces_source='flora'` on **414 of 434 species**, and 34 province lists narrowed where a published flora disagreed with the ecoregion-continuity inference (*Echinacea angustifolia* SK-only, *Yucca glauca* AB-only). The ~20 VASCAN could not settle keep **no** source and go on naming the heuristic, which for them is still the honest answer. Schema **79**, because a seed field with no column behind it is dropped on load and every page keeps saying inferred with nothing raising. F144's own countdown test — *"if this starts failing, VASCAN has landed"* — fired, and was inverted rather than deleted |
