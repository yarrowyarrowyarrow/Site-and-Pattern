...
working across both documents. **Next free ID: F130.**
...
| **F127** | Curate the 2,872 held animals — AB/prairie nativity, common name, taxon — to unlock 10,937 further GloBI edges | **open** — the 67 birds done as **F127a** in V2.60; the AB/SK nativity gate that makes the remaining 2,805 tractable shipped in V2.61 and needs its fetch run |
| **F128** | Re-fetch GloBI with `includeObservations=true`: per-study citations, the life stages that would recover the dropped larval-host records, and the body part eaten, which would recover the dropped sapsucker and browse records | **machinery shipped V2.61** — the fetch itself needs egress; see [the runbook](plans/V2.61-runbook.md) |
| **F129** | The five vegetation layers are the permaculture *forest-garden* stack, so a prairie is structurally capped at 6 of 15 — the component measures how woodland-like a design is and calls it habitat value | **open** — opened by V2.60 |
