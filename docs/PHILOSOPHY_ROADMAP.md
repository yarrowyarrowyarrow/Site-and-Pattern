...
| — | **Satellite tree detection (V2.26)** — OSM rarely maps individual trees outside city cores, so treed acreages got an empty shade map from the existing-features import and hand-marking dozens of crowns was the only way in. Now the same Esri imagery the map displays is scanned for tree crowns (excess-green vegetation gate → adaptive 2/3-class Otsu canopy split on smoothed brightness → local-texture gate → chamfer + disk-packing so a shelterbelt row resolves into individual trees), then **verified by physics** the way grass can't fake (second field pass, after a park lawn blanket-detected): a crown must be darker than the sunlit ground *around* it (ring contrast), and must cast a shadow agreeing with the photo-wide **shadow-bearing consensus** the candidates vote on (hemisphere prior applied; stand interiors inherit their blob's edge verdict). Shadow lengths then give **measured heights** — calibrated absolutely when an imported building of known height casts a legible shadow in the same photo — and crown colour tags obvious conifers/broadleafs (ambiguous stays unknown = year-round shade) for honest winter sun. Results ride the proven OSM-import tail: boundary + neighbour-margin clipping, satellite-alignment correction, dedupe against OSM/hand-marked trees, one undo step. Honesty contract (P9): positions/crown sizes measured from the photo, heights shipped as stated rough estimates, foliage left *unknown* (never invents a deciduous winter break), a failed tile fetch reported as failure — never "found 0" | `src/tree_detect.py` (Qt-free, pure stdlib) + `src/tree_detect_flow.py`, surfaced in `src/site_panel.py` (Features & Shade → existing-features import) | P8, P9 |
