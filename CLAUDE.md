...
| `src/ecoregion_map.py` | **The ecoregions, drawn (V2.48, redrawn V2.49).** Inline SVG, no script, no dependency: region fills shaded by GBIF confidence band, provincial borders from `data/provinces_prairie.geojson`, city dots. The polygons are still hand-authored (real CEC ones are an un-run download) so every drawing carries `CAVEAT`. **`scripts/draw_ecoregions.py` is where the shapes live** and it self-checks 20 pinned city lookups plus a coverage sweep before writing, because `ecoregion.lookup_ecoregions` reads these to decide what a real property gets recommended. |
